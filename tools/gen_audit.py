...
META = {
 ...
 "Mandarin":     ("Standard (Beijing)", "tʂ tɕ → ч; ʐ → ж", "tone sandhi"),
 "Hindi-Urdu":   ("Delhi", "ɦ → һ", "—"),
 "Spanish":      ("Castilian", "β → в; rhotics → р", "—"),
 "French":       ("Parisian", "ʁ → р", "—"),
 "Arabic (MSA)": ("Modern Standard Arabic", "χ → х", "—"),
 "Bengali":      ("Standard (Kolkata)", "—", "—"),
 "Portuguese":   ("Brazilian", "ʁ → р", "—"),
 "Russian":      ("Standard", "ɕː → шч", "reduction (phonemic input)"),
 "Japanese":     ("Tokyo", "ɸ → ф; ɯ → у", "pitch accent"),
 "German":       ("Standard", "—", "—"),
...
def build():
    ...
    L.append(r"\begin{longtable}{@{}p{0.24\linewidth}p{0.20\linewidth}p{0.17\linewidth}p{0.31\linewidth}@{}}")
    ...
    for lang in langs_out:
        ...
        if decl:
            hdr += " — " + "; ".join(decl)
        L.append(r"\multicolumn{4}{@{}l}{%s} \\*" % hdr)
