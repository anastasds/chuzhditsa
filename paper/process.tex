...
We position the episode against the automation literature, which evaluates language models writing papers without human taste in the loop, and argue that the interesting frontier is the opposite corner: dense, cheap, artifact-level human judgment steering machine construction. The paper closes with the authorship accounting current editorial policy requires — made reflexively awkward, and therefore explicit, by the fact that the model wrote this sentence.
\end{abstract}
