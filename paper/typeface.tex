...
A Bold whose advances do not widen is a Regular that gained weight and kept its trousers: with stroke 150 in slots spaced for stroke 90, adjacent bold \cz{о} \cz{о} overlapped outright. The Bold therefore widens every advance by the weight delta and re-centers each glyph; counters are protected by clamping stroke weight against curvature (a ring of radius $r$ carries at most $0.85r$ of stroke), which is the programmatic form of the punchcutter's counter-first discipline (Smeijers 1996). The clamp earns its keep at the small end of the curvature range: a Bold breve is an arc of radius 130 units asked to carry a 150-unit stroke — more ink than circle — and without the clamp it ships as a blob, as do the bowls of \cz{ѕ} and \cz{в} (Figure~\ref{fig:clamp}). The stroke narrows to what the curve can hold; nobody notices, which is the intended outcome.
\begin{figure}[ht]\centering
\includegraphics[width=0.60\linewidth]{figures/fig14_clamp.png}
\caption{Curvature-clamped weight. Above: Bold's full 150-unit stroke applied to the breve of \cz{ў} and the bowls of \cz{ѕ в} clogs them shut. Below: stroke clamped to $0.85r$ — the counters breathe, the weight reads unchanged.}\label{fig:clamp}
\end{figure}
...
\caption{Chuzhditsa v2.3: the Slavic base, the designed extension inventory, and the four styles — every glyph, joint, terminal, and correction in this specimen is the output of the seven hundred lines this paper described.}\label{fig:specimen}
\end{figure}
