...
\begin{abstract}
\noindent Between ordinary orthography and specialist transcription lies a layer that alphabets fill only patchily: a conventional way to write a foreign word in the native script so that a reader can recover an adequate approximation of its source pronunciation without technical training. No major Slavic Cyrillic orthography occupies that layer — foreign words enter through ad hoc transcription, with systematic indeterminacy as the result (Bulgarian \cz{Вашингтон}~$\sim$~\cz{Уошингтън}, Данчев 1982). We present \emph{Chuzhditsa} (Bulg.\ чуждица, `foreignism'), a computable, Cyrillic-based \emph{phonological citation register} for foreign words, names, and pedagogical transcription. Japanese katakana proves that a visually distinct foreignness register can operate at the scale of a national orthography — though katakana adapts loans \emph{into} native phonology, where a citation register must extend the inventory to \emph{preserve} source contrasts.

Chuzhditsa supplies (i) a compositional grammar of diacritic operations deriving letters for non-Slavic phonemes from attested Cyrillic material — revivals, imports, and rule-derived residue; (ii) a prosodic layer for stress, tone, and length; and (iii) a register-marking typeface whose substitution and positioning rules (GSUB/GPOS) are the orthography's fusion and attachment rules, executable rather than merely described. We evaluate coverage against twenty languages under declared reference varieties and a three-way classification (exact, conventionally merged, declared loss), generalize the Bulgarian pilot to the Slavic Cyrillic superset with an external stress test on Albanian, and offer the laryngeal series' articulatory-to-visual gradient as a candidate heuristic for alphabet extension, not a demonstrated law.
\end{abstract}

\section{Introduction}

Every literate community borrows words faster than its orthography naturalizes them — an instance of the general problem of fit between a writing system and a changing language (Coulmas 2003). What most orthographies lack is a \emph{productive} middle layer between everyday spelling and technical transcription: a conventional register for writing a foreign word so that its source pronunciation is recoverable. The layer is not empty — monolingual dictionaries fill it with pronunciation respelling, Japanese with furigana glosses, and the zonal constructed language Interslavic with an extended Cyrillic that already revives the yuses (Merunka 2018) — but no major Slavic Cyrillic orthography has an inventory-\emph{extending}, register-marked device of this kind.
